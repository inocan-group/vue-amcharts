<template>
  <div class="curve-column-series"></div>
</template>
...
export default defineComponent({
  name: 'CurveColumnSeries',
  props: {
    ...seriesProps,
  },

  setup(props: IDictionary, context: SetupContext) {
    const series: Ref<CurveColumnSeries> = ref(new CurveColumnSeries())
    ...
    register(ChartType.series, props.id, CurveColumnSeries, series)
    ...
    onChartConfig((chart: IChart) => {
      ...
      if (props.tooltipText) {
        console.warn(
          `You have configured tooltip text for the ${props.name} CurveColumnSeries component but there is no Cursor on this chart so it will not be displayed!`,
        )
      }
    })
